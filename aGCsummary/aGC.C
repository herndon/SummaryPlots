	...
	if (chanName[ii]!=currentName)
	  {
	    ...
	    if (atgc) txtY[ntxt] = nPlotChan - (nBin_) +1.2*DY_;
  ...
  // Create ledgend
	...
	for (int ii=0; ii<numberExperiments;ii++){
	  if (types&baseTwo){
	    ...
	    _dataTotPoint.SetPoint(1, xx_, yy_-dyy_*exp*1.0 );
	    ...
	    TMarker _dataPoint( xx_, yy_-dyy_*exp*1.0, kFullCircle );
	    _dataPoint.SetMarkerSize(markerSize);
